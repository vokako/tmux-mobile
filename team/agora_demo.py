...
KICK = ("你已接入 agora 群聊（协作规则见 AGENTS.md）。"
...
def full_prompt(role, goal, backstory):
    ...
    return (f"你是「{role}」。\n目标：{(goal or '').strip()}\n背景：{(backstory or '').strip()}\n"
            "你和其他 agent、以及一位人类，在共享的『agora 群聊』里协作（通过 @agora 工具）。"
            "请始终用中文交流，消息保持简短。")
...
def _kiro(name, role, goal, backstory, manage, url, demo, workspace, model):
    ...
    agora = ["@agora"] if manage else [f"@agora/{t}" for t in WORKER_TOOLS]
    conf = {
        "name": name,
        ...
        "mcpServers": {"agora": {"url": f"{url}/mcp", "headers": {"x-agent": name}}},
...
def _claude(name, role, goal, manage, url, demo, model):
    ...
    mcpfile.write_text(json.dumps({"mcpServers": {"agora": {
    ...
    env = {}
    m = model or "sonnet"
    disallow = "" if manage else "--disallowedTools mcp__agora__hire mcp__agora__fire "

...
Hover / unfold / highlight (#86, 2026-09-04): resting on a window chip opens the ONE hover card (`use:hoverInfo` on the `.win-chip` wrapper — `index:name`, command, pane count, agent tag; the chip's own `title` prop is left unset so the card is not doubled — AgentChip still falls back to its label as a native title, a `ui/` change to make), and the to-tail button's card is a single note ("Back to the newest output"). Nothing unfolds and no highlight travels here: the chip strip scrolls and its chips are AgentChips, so the active chip is not a `.slide-ind` candidate, and the terminal's box stays untouched (principle 9 — the card is its own fixed layer and the action only adds listeners).

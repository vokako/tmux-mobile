...
[motion.md](motion.md) applied to Team, AgentGrid and TeamTemplates. The roster chips are keyed by name, pop in (`.appear-pop`) and `animate:flip` on `moveMs()` when the roster reorders; a roster dot changes colour and halo on `--t-fast`, never opacity. Only a LIVE message bubble rises in: the push handler records the id in a plain `liveIds` set before appending, and a history load clears it, so two hundred rows never animate together (principle 13). The phone's collaboration panel rises in (its height is inline and instant — the drag handle is the animation). The header buttons' `.on`/`:active` clothes cross-fade. AgentGrid cells cross-fade border colour and ring only, and the maximized `.ag-expanded` may only FADE — both are xterm ancestors (principle 9). TeamTemplates' three disclosures are one `chevron-right` in `.chev` that turns, and their bodies fade in with no slide. The compose grip no longer animates its width (layout is never animated). Deliberately left alone: the `swapSides` remount (a layout bug, not a motion gap) and the `.tpl-modal` sheet (the shared sheet grammar's job).

Hover / unfold / highlight (#86, 2026-09-04): a roster chip's hover card (`use:hoverInfo`, principle 16) shows state, role, backend, model and room — the roster row plus the desired roster's launcher `spec` for the same name, data the grid already holds; the chip's native `title` is gone. The roster strip unfolds once per room: `revealRoster()` puts `.reveal` on `.team-header-scroll` when a room's first roster lands and drops it after `moveMs() * 4`, so a later joiner pops in (`.appear-pop`) instead of rising. TeamTemplates' template list is the ONE travelling highlight (`use:slideIndicator` + `.slide-pill`; the item only changes colour, the pill carries the accent frame and wash) and wears `.reveal` so the list unfolds when the sheet opens. The header's collab/swap buttons are independent toggles, not a segmented group, so no highlight travels between them.

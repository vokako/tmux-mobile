...
`notifiable()` filters first-load/watched batches, own messages, lifecycle narration, and — by LEVEL — replies and ambient status updates. The level is three nested rungs, `done ⊂ replies ⊂ all`: a board move to review/done (and historical `[tmm done]` rows) is news at every rung; a plain final reply from `replies` up; a `[tmm status]` progress note only at `all`. The default (`replies`) includes every hook-captured final response.
...
**The LEVEL is three nested rungs**: Settings → Notifications → `done` (board review/done and historical `[tmm done]`) ⊂ `replies` (default; every agent final reply) ⊂ `all` (plus `send --status` progress notes).

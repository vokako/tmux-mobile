...
**Every transient layer follows that rule** (owner, 2026-08-22: "在其他操作之后应该自动隐藏 不应该一直常驻显示"): the recipient picker closes on outside pointerdown + Escape (the message action row followed the same rule until the whole message menu was retired, board #48), and a tap outside the composer parks the `/` palette exactly like Escape (paletteOff, reset by the next text change). Modal dialogs keep their backdrop-click dismissal. `project_delete` / `projects::delete` closes the session, removes every `<path>/.tmm/agents/<name>/` and forgets the row (slots cascade) — in that order, and the close must SUCCEED first (2026-09-03 review): a session that survives with its homes deleted and its row gone is an untracked session full of windows nobody owns, which `auto_adopt_once` re-adopts as a new project within 120 s, so a delete that ignored `down`'s error resurrected what it was asked to forget. `agent_remove` follows the same rule for its window: a kill that fails is an error before the slot and home go, not a warning after. But in the chat UI it is reachable only through the RECYCLE BIN (owner, 2026-08-21, same two-step rule as messages): the header's Delete does `project_down` + `project_archive` and the project waits in a folded "回收站" section at the sidebar's bottom (hidden while empty), where restore is free and the confirmed `project_delete` is the only irreversible step; archived is never re-adopted, which keeps the bin stable. Both deliberately spare the user's files (we only delete inside `.tmm/agents/`) and the chat history (the room is the record, keyed by session name).
...
### Motion: a project that changes state moves, nothing swaps

[motion.md](motion.md) applied to `Projects.svelte`. `sortRows` ranks live projects first, so `up`/`down` changes a card's rank — the cards are keyed by id and carry `animate:flip={{ duration: moveMs() }}`, the most meaningful reorder in the app: the card you just closed travels down instead of the list repainting. A card `.appear`s when it mounts (a new project, or the section expanding — the expand itself is a cut, no slide). The section's caret is ONE `chevron-right` in `.chev` with `class:open={!collapsed}` (principle 4 — it turns, it is not swapped for a `chevron-down`). The live border, the dense hover/open wash and the state dot's colour cross-fade on `--t-fast` (a dot changes colour, never opacity); the declared-windows dim on `--t-move`. The error line fades in.

Hover / unfold / highlight (#86, 2026-09-04): a card's hover card (`use:hoverInfo` on `.proj-main`, principle 16) shows the path, the state (open/closed), the agents as "n live · n stopped" (declared agent slots against the live agent panes `chipsFor` already knows) and the age through `ageLabel` — the card's own helpers, never a second formatter; the path's native `title` is gone, the `aria-label` on the group stays. The cards unfold once: `firstFill` flips on the first answer with rows and puts `.reveal` on the section; later loads keep the keyed cards' nodes. There is no segmented control here, so no highlight travels.

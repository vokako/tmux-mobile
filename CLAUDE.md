...
10. **A managed agent is defined by its isolated home** (`<ws>/.tmm/agents/<name>/`, `projects::managed_home`); its model and effort live in its CONFIG, never on the launch line; the launch recipe (env + PATH + identity) is replayed on restart; the app-wide `<config>/AGENTS.md` is the first block of every prompt (`tmm prompt`). → [agents-overview.md](docs/design-docs/features/agents-overview.md), [tmm-cli.md](docs/design-docs/features/tmm-cli.md#the-app-wide-instructions-configagentsmd-2026-09-04)
11. **Final replies follow one reply edge**: hooks record every managed agent's final response and return it once to the agent that opened the turn; `[reply]` never creates a reverse edge. Status is DERIVED from turn edges — pane activity is not work, Claude's `idle_prompt` is not an ask. → [agent-status.md](docs/design-docs/features/agent-status.md)

...
8. **The keyboard is an overlay for agent TUIs** (`.keep-rows`), a resize for everything else; printable keys bypass xterm's keydown so CJK IMEs work; only `unlockKeyboard()` and friends may touch `kbLocked`; on the macOS app Escape is answered natively (`cancelOperation:` no-op on the webview) and the guard watches `document.hasFocus()`, never just the DOM. → [terminal-keyboard.md](docs/design-docs/pages/terminal-keyboard.md)
